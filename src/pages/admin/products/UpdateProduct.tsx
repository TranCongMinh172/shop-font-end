...
import { createProductDetails, removeProductDetail } from "../../../services/product-detail.service";
...
const UpdateProduct = () => {
    ...
    const formikProductDetail = useFormik({
        ...
        onSubmit: async (values: ProductDetailDto, { resetForm }) => {
            ...
            setOpenBackdrop(true);
            try {
                const response: ResponseSuccess<ProductDetailModel> = await createProductDetails(values);
                    ...
                    <TableBody>
                        {productDetail.map((productDetailModel: ProductDetailModel, index: number) => (
                            <TableRow key={index} sx={{
                                ...
                                <TableCell >{colors.filter(color => productDetailModel.color.id === color.id)[0]?.name}</TableCell>
